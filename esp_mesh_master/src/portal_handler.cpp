...
static uint8_t activeDeviceId = 0;
...
void enterPortalMode(uint8_t deviceId)
{
  ...
  /* Only stop bridge if it was started (skip on post-OTA portal boot) */
  ...
  char ssid[32];
  snprintf(ssid, sizeof(ssid), "OTA-MASTER-%u", (unsigned int)activeDeviceId);
