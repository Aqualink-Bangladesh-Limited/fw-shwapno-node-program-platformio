...
static uint8_t handleReadRequest(uint8_t *requestBuffer, uint8_t *responseBuffer);
static uint8_t handleWriteSingleRequest(uint8_t *requestBuffer, uint8_t *responseBuffer);
static uint8_t handleWriteMultipleRequest(uint8_t *requestBuffer, uint8_t *responseBuffer);
...
static uint16_t readMasterRegister(uint16_t regAddress)
{
  switch (regAddress)
  {
  case MASTER_REG_WIFI_RSSI:
    if (WiFi.status() == WL_CONNECTED)
      return static_cast<uint16_t>(WiFi.RSSI() * -1);
    return 0;
  default:
    return 0xFFFF;
  }
...
static uint8_t handleReadRequest(uint8_t *requestBuffer, uint8_t *responseBuffer)
{
  ...
  {
    uint16_t value = readMasterRegister(startAddress + i);
                        ...
                        uint8_t *responseBuffer, int responseMaxLen)
{
  ...
  debugLog("Master Modbus response (%u bytes)", (unsigned)pduLen);
  return pduLen;
}
